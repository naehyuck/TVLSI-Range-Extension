...
%We summarize the comparison of the driving range and driving time on the six benchmarking roads in Fig.~\ref{fig:no_deadline_bar}. Y-axis denotes the range and driving time compared with the least-energy-constant-velocity driving. Blue-colored bars indicate the least-energy-constant-velocity driving, which is the baseline. Green-colored bars indicate the minimum-energy-velocity planning, respectively.
%
...
%%Histogram analysis
%\textcolor{red}{We quantize the velocity and acceleration values and make a histogram as shown in Fig.~\ref{fig:histogram} to analyze population of velocity and acceleration for various energy-aware-velocity planning methods. For the EDP-aware-velocity planning, it is used when the deadline is the most important concern. Therefore, it is guided to drive at a speed of 75-130 km/h mainly for all road slope. The $E^2DP$- and $E^3DP$-aware-velocity planning methods are used when the remaining battery state of charge is more concerned than the deadline. It is guided to drive at an energy-efficient speed along a road slope. Particularly, the $E^3DP$-aware-velocity planning method is used when the state of charge is more concerned, and the driver should find the nearby charging station. It is guided to decelerate to 25-40 km/h when climbing hills to further reduce energy consumption.}
